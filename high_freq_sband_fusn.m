function fused_sband=high_freq_sband_fusn( sband1, sband2 )
    % high frequency sub-bands fusion for NSCT transformed images
    % 
    % to accomplish eq.9, eq.10, eq.11 and eq.12
    % of the paper:
    % CT and MR Image Fusion Scheme in Nonsubsampled Contourlet Transform Domain
    % 
    % input:
    %   sband1, sband2: high frequency sub-bands from images to be fused
    % output:
    %   fused_sband: the fused high frequency sub-band
    % 
    % 
    ...
    act_sband1 = weighted_sum_modified_laplacian(sband1);
    act_sband2 = weighted_sum_modified_laplacian(sband2);
